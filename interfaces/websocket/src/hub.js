...
function createDefaultGraph(task) {
    if (!task) {
        return {
            id: 'workspace-graph',
            nodes: [],
...
function normalizeGraphNode(nodeId, value, fallbackStatus) {
    if (!isRecord(value)) {
        return {
            id: nodeId,
            label: nodeId,
...
function deriveGraphFromTask(task) {
...
export class DefaultWorkspaceHub {
    workspaceId;
    ...
    getTask(taskId) {
        return this.tasks.get(taskId) ? clone(this.tasks.get(taskId)) : null;
    }
    listTasks() {
    ...
    setConnectionState(activeConnections, timestamps) {
...
export function createWorkspaceHub(options) {
